...
def shortest_string(strings):
    ...
    for k in range(1, len(strings)):
        if len(strings[k]) < len(strings[short_string]):
            short_string = k
    return strings[short_string]
...
def number_of_stutters(s):
    ...
    for k in range(len(s) - 1):
        if s[k + 1] == s[k]:
            number_of_stutter = number_of_stutter + 1
    return number_of_stutter
...
def count_same(sequence1, sequence2):
    ...
    for k in range(len(sequence1)):
        if sequence1[k] == sequence2[k]:
            number_indices = number_indices + 1
